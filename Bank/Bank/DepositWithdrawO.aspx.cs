...
namespace Bank
{
    public partial class WebForm27 : System.Web.UI.Page
    {
        ...
        {
            DepositWithdraw master = (DepositWithdraw)this.Master;
            TextBox TextBoxAmount = (TextBox)master.FindControl("TextBoxAmount");
            LoadDB("Customer", "CustomerId", 10000000, 1);
            SqlDataAdapter adapter = (SqlDataAdapter)Cache["Adapter"];
            DataSet ds = (DataSet)Cache["DataSet"];
            DataRow[] dr = ds.Tables["Customer"].Select("CustomerId = " + Convert.ToInt32(TextBoxOther.Text));
            int status = Convert.ToInt32(TextBoxOther.Text);
            foreach (DataRow row in dr)
            {
