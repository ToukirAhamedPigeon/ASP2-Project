...
namespace Bank
{
    public partial class Search : System.Web.UI.MasterPage
    {
        ...
        protected void TextBoxSearchCust_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            /*if (System.Text.RegularExpressions.Regex.IsMatch(TextBoxSearchCust.Text, "^[0-9]*$"))
            {
                string sql = "SELECT * FROM Customer where CustomerId ="+TextBoxSearchCust.Text+ "'";
                LoadDB(sql, "Customer", "CustomerId", 10000000, 1);
                SqlDataAdapter adapter = (SqlDataAdapter)Cache["Adapter"];
                DataSet ds = (DataSet)Cache["DataSet"];
                GridViewCust.DataSource = ds.Tables["Customer"];
                GridViewCust.DataBind();
            }
            else if (System.Text.RegularExpressions.Regex.IsMatch(TextBoxSearchCust.Text, "^[a-zA-Z]*$"))
            {
                string sql = "SELECT * FROM Customer where CustomerName ="+TextBoxSearchCust.Text + "'";
                LoadDB(sql, "Customer", "CustomerId", 10000000, 1);
                SqlDataAdapter adapter = (SqlDataAdapter)Cache["Adapter"];
                DataSet ds = (DataSet)Cache["DataSet"];
                GridViewCust.DataSource = ds.Tables["Customer"];
            ...
            Response.Redirect("~/InfoSearchCustomer.aspx?Id="+TextBoxSearchCust.Text);
            Session["Visit"] = Convert.ToInt32(TextBoxSearchCust.Text);
        }

        protected void ButtonAdmin_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/InfoSearchAdmin.aspx?Id=" + TextBoxSearchAdmin.Text);
            Session["Visit"] = Convert.ToInt32(TextBoxSearchAdmin.Text);
        }
    }
}